...
DearDaria.initHeroCarousel = function (images) {
  ...
  wrap.innerHTML = images.map((item, i) =>
    `<img class="hero-carousel-img${i === 0 ? ' active' : ''}${item.extraClass ? ' ' + item.extraClass : ''}" src="${DearDaria.imgUrl(item.src)}" alt="${item.altKey ? DearDaria.t(item.altKey) : ''}" loading="${i === 0 ? 'eager' : 'lazy'}" draggable="false" style="background:${item.bg || 'var(--ivory-deep)'};${item.pos ? ` object-position: ${item.pos};` : ''}${item.scale ? ` transform: scale(${item.scale});` : ''}">`
  ).join('')
